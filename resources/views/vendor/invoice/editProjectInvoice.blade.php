      ...
      <div class="col-md-11">
            <div class="card card-success shadow-sm" >
              <div class="card-header">
                <h2 class="card-title">Original system information for Project 
                Project @if($wo->client) {{$wo->client->code}} - @endif{{str_pad($wo->id, 4, "0", STR_PAD_LEFT )}} - {{$stage->project->id}}'s invoice </h2>
           ...
           <div class="card card-primary col-md-11" id="edit_div_invoice">
              <div class="card-header">
                <h3 class="card-title">Edit Current Invoice Details  </h3>
              ...
              @csrf
                ...
                <div class="row">
                <input type="hidden" value="{{$invoiceItem->id}}" name="invoiceItemId" id="invoiceItemId">
                ...
                <div class="row">  
                  <div class="form-group col-md-6">
                    ...
                    <input type="number" min="0" step="1" class="form-control" name="unit_count"
                     value="{{$invoiceItem->unit_count}}" id="unit_count" placeholder="Enter 0 if Target " required>
                  ...
                  <div class="form-group col-md-6">
                    ...
                    name="rate" id="rate" value="{{$invoiceItem->rate}}" required>
                  </div>
                </div>
                <div class="row">
                ...
                <input id="total" name="total" type="number" class="form-control" value="{{$invoiceItem->unit_count * $invoiceItem->rate}}" required readonly>
              </div>
                </div>
            ...
            <button type="button" class="btn btn-primary">
              <a href="{{route('vendor.view-vendorInvoice', $invoiceItem->invoiceId)}}"> 
                ...
                <button class="btn btn-success submitInvoice">save</button>
                </div>
        </form>
       </div>
       </div> 
...
<script>
$(function () {
    var rateUnit = "{{$invoiceItem->rate_unit}}"
  $("#rate_unit option[value='"+rateUnit+"']").attr("selected", "selected");
    ...
    $('#editInvoice-form').submit(function(e) {
      document.body.style.cursor='wait';           
       ...
       e.preventDefault();
        ...
        $.ajax({
                data: formData,
                url: "{{route('vendor.edit-workInvoice') }}",
                type: 'POST',
                contentType: false,
                processData: false,
                success: (response) => {
